...
/* NOTE: Regex support removed - will be reimplemented with Hyperscan */
...
/* NOTE: Regex cache removed - will be reimplemented with Hyperscan */
...
#undef CHECK_PRAGMA
   ...
   * 1. block_exact      → IPSetTerminate (IPv4 + IPv6 direct response)
   * 2. block_wildcard   → IPSetDNSBlock (DNS Forward to blocker)
   * 3. fqdn_dns_allow   → IPSetDNSAllow (DNS Forward to real DNS)
   * 4. fqdn_dns_block   → IPSetDNSBlock (DNS Forward to blocker)
   *
   * Note: block_regex removed - will be reimplemented with Hyperscan
  ...
  /* block_exact (Domain) → IPSetTerminate */
                     ...
                     char **ipv6_out)  /* OUT: IPv6 address or NULL */
{
  ...
  {
    /* FIX: Get termination IPs from static config strings (set via option.c) */
    char *v4_str = db_get_ipset_terminate_v4();
    char *v6_str = db_get_ipset_terminate_v6();

    /* Return IPv4 address (already in TLS buffer from getter) */
    if (ipv4_out && v4_str)
    {
      *ipv4_out = v4_str;
    }

    /* Return IPv6 address (already in TLS buffer from getter) */
    if (ipv6_out && v6_str)
    {
      *ipv6_out = v6_str;
    }

    printf("block (v4.0): %s → %s (ipv4=%s, ipv6=%s)\n", name,
           ipset_type == IPSET_TYPE_TERMINATE ? "TERMINATE" : "DNS_BLOCK",
           v4_str ? v4_str : "none", v6_str ? v6_str : "none");
    return 1;  /* Blocked */
  }
...
/* NOTE: Regex functions removed - will be reimplemented with Hyperscan */

/* Schema v4.0: New lookup function with priority order
 * Returns IPSET_TYPE based on lookup result:
 *   1. block_exact     → IPSET_TERMINATE
 *   2. block_wildcard  → IPSET_DNS_BLOCK
 *   3. fqdn_dns_allow  → IPSET_DNS_ALLOW
 *   4. fqdn_dns_block  → IPSET_DNS_BLOCK
